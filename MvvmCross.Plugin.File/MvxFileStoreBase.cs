    ...
    : IMvxFileStore, IMvxFileStoreAsync
{
    ...
    {
        return await TryReadFileCommonAsync(path, async stream => await readMethod(stream).ConfigureAwait(false)).ConfigureAwait(false);
    }

    public async Task<bool> TryReadBinaryFileAsync(string path, Func<Stream, CancellationToken, Task<bool>> readMethod, CancellationToken cancellationToken)
    {
    ...
    public async Task WriteFileAsync(string path, Func<Stream, CancellationToken, Task> writeMethod, CancellationToken cancellationToken)
    {
        await WriteFileCommonAsync(path, async stream =>
        {
